...
Mean_compare_probability::Mean_compare_probability(
	CowardVariables & cowa_store,
	const string	& task_string
	) : Property(cowa_store)
{

// Mean_compare_probability 	0 pl_dist_0 0  	1 


// Mean_compare_probability  0	DUMB	W5_01234_dx2 0     -2 2  0.1  1 
	istringstream ist( task_string );

	ist >> name_ ;
	assert ( name_ == "Mean_compare_probability" ) ;
...
//	double this_cluster_sigma_;
	...
	else 
	{
		...
		cout << "Mean_compare_probability() ERROR: can't associate Frequency_extrapolation object for name " << endl;
	//	log  << "Mean_compare_probability() ERROR: can't associate Frequency_extrapolation object for name " <<  frequency_map_name_ << endl;
		exit (-1);
	}
...
Property* Mean_compare_probability::
clone (const string & task_string) const
{
	return new Mean_compare_probability(cowa_store_, task_string);
}
	
double    Mean_compare_probability::
calc_value(const int   position_in_chain)
{
	...
	vector <double>   const & squared_sum_array = cowa_store_.pull_fcc_ [index_in_frequency_pull_ ].tot_squared_distance_to_clusters_sum	() ;


		double s_x  = average_array[claster_index_];
		double sx_sx  = average_array[claster_index_]*average_array[claster_index_];
		double s_x2 = squared_sum_array[claster_index_];
		double n_s_x2 = total_sample_size * squared_sum_array[claster_index_];
	...
	for ( int ii = start; ii < end; ii++ ) 
	{
		...
		if ( occurence[ii] ) 
		{
			...
			double t = sqrt (current_number ) * ( total_average_value *  ( 1 - value_shift_koef_ ) -  current_average  ) / sigma ;
