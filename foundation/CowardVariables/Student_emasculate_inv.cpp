...
Student_emasculate_inv::Student_emasculate_inv(
	CowardVariables & cowa_store,
	const string	& task_string
	) : Property(cowa_store)
{

// Student_emasculate_inv 	0 pl_dist_0 0  	1 


// Student_emasculate_inv  0	DUMB	W5_01234_dx2 0     -2 2    1 
	istringstream ist( task_string );

	ist >> name_ ;
	assert ( name_ == "Student_emasculate_inv" ) ;
	...
	else 
	{
		...
		cout << "Student_emasculate_inv() ERROR: can't associate Frequency_extrapolation object for name " << endl;
	//	log  << "Student_emasculate_inv() ERROR: can't associate Frequency_extrapolation object for name " <<  frequency_map_name_ << endl;
		exit (-1);
	}
}

Property* Student_emasculate_inv::
clone ( 
	const string			& task_string) const
{
		return new Student_emasculate_inv(cowa_store_, task_string );
}
	

double Student_emasculate_inv::
	...
	//vector < vector < double > >   & sophisticated_variables  )
{
	...
	//vector < double > average_array = cowa_store_.pull_fcc_[index_in_frequency_pull_ ].tot_inverse_distance_to_clusters_sum();
	...
	{
	//	vector < double> current_array = pull_fcc [index_in_frequency_pull_ ].distance_to_clusters_sum()[ii];
