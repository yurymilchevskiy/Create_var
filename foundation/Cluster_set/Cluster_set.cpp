	...
	sheduler_							(0)
{
	...
	string path_to_current_schedule = configuration.option_meaning("Path_to_Cluster_set")  +  string(cluster_set_mame) + string ("/") + 		string ("sheduler") ;
	...
	switch ( run_mode  )
	{
		...
		case COMMON_USAGE_CLUSTER_SET_MODE: 
			init_claster_motif() ;
			break;
		case FILL_UP_MODEL_CLUSTER_SET_MODE:

				// Это раньше был общший кусок и располагался в самом начале
		sane_metrics_mame = sheduler_->option_meaning("SANE_METRICS_NAME");
		sane_metrics_mame_ = sheduler_->option_meaning("SANE_METRICS_NAME");
		sane_metrics_mame_  = sane_metrics_mame;

		sa_me_ = new  Sane_metrics   (  
			sane_metrics_mame_,
			SANE_METRICS_COMMON_USAGE);

		fragment_base_subtle_name = sa_me_->get_fragment_base_subtle_name();
		 zip_factor = sa_me_->get_zip_factor();
		 shift	   = sa_me_->get_shift();
...
// Это раньше был общший кусок
		...
		//	memset (subsample_fragments_,0,sizeof(double*)	 *subsample_size_);
...
vector <int>  Cluster_set::algorithm_2 (
	double upper_dist,				
	double lower_dist,				
	double step_dist,				
...
//	cout << "***********" << endl;
//	for (int ttt=0;ttt<choosen_indexes.size(); ttt++)
//		cout << choosen_indexes [ttt] << endl;
...
//	double **bs_fragments = new double* [choosen_indexes.size()] ;	memset (bs_fragments,0,sizeof(double*) * choosen_indexes.size());
//	for ( int ii=0; ii < choosen_indexes.size(); ii++  )
//		bs_fragments [ii]  = new double [length_*9];

//	for ( int ii=0; ii < choosen_indexes.size(); ii++  )
//	{
//		int  check = zip_it_->get_global_index( choosen_indexes[ii] );
//		fragment_base_->get_coord ( zip_it_->get_global_index( choosen_indexes[ii] ), bs_fragments [ii] );
//		
//	}	
...
//	for (  int ii=0;ii< total_fragment_number ; ii++ ) 
	...
	//	 fragment_base_->get_coord ( ii , cur_fragment  );
		 ...
		 for (int kk=0; kk< choosen_indexes.size();kk++ )
		 {
		//	memcpy(bs_fragment_lcopy,bs_fragments [kk],length_*9*sizeof(double));
...
// choosen_indexes - relating to total ( not reduced base) !!!
	string path_to_current_protocol = configuration.option_meaning("Path_to_Cluster_set")  +  string(cluster_set_mame_) + string ("/") + 		string ("protocol") ;
...
//			zip_it_->get_global_index( claster_diversity[ii].in_data_base_index() ), 
//			cur_fragment );
...
//		string  pdb_chain_ID		= cur_fragment->get_pdb_chain_ID();
		...
		int global_index = zip_it_->get_global_index (claster_diversity[ii].in_data_base_index() );
...
vector < vector <double> >  Cluster_set::  // дублируется часть кода из plain_claster_show_subsample_version ну извитите. 
...
optimize_clasterization () 
{
	...
	string protocol_file_name			= host_dir_ + 	sheduler_->option_meaning("PROTOCOL_FILE_NAME");
	string plain_claster_show_file_name = host_dir_ + 	sheduler_->option_meaning("PLAIN_CLASTER_SHOW_FILE_NAME");
...
void Cluster_set::subtle_claster_show_for_whole_base ( 
 	const string & output_file_name,
	vector < Single_cluster_record > & claster_diversity )
{
